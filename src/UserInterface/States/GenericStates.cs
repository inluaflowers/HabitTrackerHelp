...
public class InputState<TParentState, T1Input, T2Input>(string title, string itemKey, Func<InputState<TParentState, T2Input>> stateFactory) : BaseState
    where TParentState : BaseState
{
    private readonly string _title = $"Choose a {title} for {itemKey} ";
    public override void Display()
    ...
    public override void Display()
    {
        ...
        Context.Cache.ActionCache.Add(itemKey, () =>
            Context.TransitionTo(
                new InputState<TParentState, string, float>("Name",  itemKey, ()  =>
